...
from app.schemas.content import ContentList, ContentListItem, ContentResponse, RevisionRequest
...
def get_chief_editor_dashboard(
    ...
    """
    in_review_count = db.query(func.count(Content.id)).filter(
    ...
    needs_revision_count = db.query(func.count(Content.id)).filter(
...
def approve_content(
    ...
    if content.status != ContentStatus.IN_REVIEW:
        raise HTTPException(
            ...
            detail="Content is not in review"
        )
